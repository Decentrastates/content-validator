  ...
  validate: async ({ deployment, externalCalls }, logs) => {
    ...
    const findBlocksForTimestamp = async (
      ...
      try {
        ...
        logs?.error(`Error fetching the block number for timestamp`, { timestamp, error: (error as any)?.message })
        throw error
      }
    }
    ...
    const checkItemExistence = async (
      ...
      try {
        ...
        return (
          (await existsOnBlock(blockNumberAtDeployment)) || (await existsOnBlock(blockNumberFiveMinBeforeDeployment))
        )
      } catch (error) {
        logs?.error(`Error checking item existence (${urn}).`)
        return false
      }
    }
